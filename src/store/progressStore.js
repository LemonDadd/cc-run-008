...
import { uid, weekKey } from '../lib/util.js';
import { BASE_COLORS } from '../data/colors.js';
import { useAppStore } from './appStore.js';
import { sfx } from '../lib/audio.js';

// 取某个孩子本周的观察任务（同周多条时取最新一条）
const pickWeeklyTask = (rows) =>
  rows
...
export const useProgressStore = create((set, get) => ({
  ...
  weeklyTask: null, // 本周观察任务（仅当前孩子自己的）
  records: {
    mixing: [],
    matching: [],
    coloring: [],
  ...
  async loadFor(profileId) {
    const [colorProgress, mixing, matching, coloring, diary, weekly] = await Promise.all([
    ...
    set({
      ...
      weeklyTask: pickWeeklyTask(weekly),
      records: {
  ...
  resetForCurrent() {
    set({ profileId: null, colorProgress: [], weeklyTask: null, records: { mixing: [], matching: [], coloring: [], diary: [] } });
  },

  // 家长发布后，当前孩子首页同步刷新本周任务
  async refreshWeeklyTask() {
    const pid = get().profileId;
    if (!pid) return;
    set({ weeklyTask: pickWeeklyTask(await idbGetByProfile('weeklyTasks', pid)) });
  },

  // 孩子在首页点“我完成啦”：打勾 + 1 颗星（只记在当前孩子名下）
  async completeWeeklyTask() {
    const task = get().weeklyTask;
    const pid = get().profileId;
    if (!task || task.done || !pid) return false;
