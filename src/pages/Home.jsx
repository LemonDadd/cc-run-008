...
// 新入口：放在六个原模块下面，并标上“新”
const NEW_MODULES = [
  { to: '/mood', icon: '😊', title: '心情课', subtitle: '颜色也有心情', color: '#EC4899', isNew: true },
  { to: '/temperature', icon: '🌡️', title: '色温', subtitle: '暖暖和凉凉', color: '#F6A21E', isNew: true },
];
...
export default function Home() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 pb-14">
      ...
      {/* 家长布置的本周观察任务：只显示当前这个孩子自己的 */}
      {weeklyTask && (
        <div className="card-kid mb-8 flex flex-wrap items-center gap-4 p-5">
          <span className="text-5xl">📝</span>
          <div className="min-w-0 flex-1">
            <div className="font-body text-slate-400">本周小任务（家长布置）</div>
            <div className="font-btn text-slate-700">{weeklyTask.text}</div>
          </div>
          {weeklyTask.done ? (
            <span className="rounded-full bg-green-50 px-6 py-3 font-btn text-green-500">
              ✓ 已完成 ⭐+1
            </span>
          ) : (
            <KidButton variant="green" data-testid="task-done" onClick={finishTask}>
              我完成啦 ⭐
            </KidButton>
          )}
        </div>
      ...
      {/* 新课入口：放在六个原模块下面 */}
      <h2 className="mt-10 font-cname text-slate-600" style={{ fontSize: 30 }}>
        ✨ 新课来啦
      </h2>
