...
import { downloadJson, formatDate, formatTime, uid, weekKey } from '../lib/util.js';
...
function PanelBody() {
  ...
  const [weeklyTaskRow, setWeeklyTaskRow] = useState(null);
  const [taskText, setTaskText] = useState('');

  // 本周观察任务：当前选中的这个孩子本周的任务（直接查库，当前/非当前孩子都适用）
  useEffect(() => {
    let alive = true;
    (async () => {
      const rows = await idbGetByProfile('weeklyTasks', pid);
      const row =
        rows
          .filter((r) => r.week === weekKey())
          .sort((a, b) => b.createdAt - a.createdAt)[0] || null;
      if (alive) setWeeklyTaskRow(row);
    })();
    return () => {
      alive = false;
    };
  ...
  // 发布/替换本周任务：同一孩子同一周只留一条，重新写会重置完成状态
  const saveWeeklyTask = async () => {
    const text = taskText.trim();
    if (!text) return;
    const rec = {
      id: weeklyTaskRow?.id || uid('wt_'),
      profileId: pid,
      text,
      week: weekKey(),
      done: false,
      doneAt: null,
      createdAt: weeklyTaskRow?.createdAt || Date.now(),
    };
    await idbPut('weeklyTasks', rec);
    setWeeklyTaskRow(rec);
    setTaskText('');
    if (isCurrent) await progress.refreshWeeklyTask();
    sfx.correct();
  };
              ...
              <div className="mt-5 space-y-5">
                ...
                <div className="card-kid p-6">
                  ...
                  <p className="mt-1 font-body text-slate-500">
                    写一条小任务（例如“找出三种暖色”），会显示在「{profile.nickname}」的首页，孩子点完成后得 1 颗星。
                  </p>
                  {weeklyTaskRow && (
                    <div className="mt-3 flex items-center gap-3 rounded-2xl bg-amber-50 p-4">
                      <span className="min-w-0 flex-1 font-body text-slate-700">{weeklyTaskRow.text}</span>
                      <span className={`shrink-0 font-body ${weeklyTaskRow.done ? 'text-green-600' : 'text-slate-400'}`}>
                        {weeklyTaskRow.done ? '✓ 孩子已完成' : '进行中'}
                      </span>
                    </div>
                  )}
                  <div className="mt-4 flex gap-3">
                    <input
                      ...
                      placeholder="例如：找出三种暖色"
                      className="min-w-0 flex-1 rounded-2xl border-4 border-amber-200 px-5 py-3 text-xl outline-none focus:border-amber-400"
                    />
                    <KidButton
                      variant="primary"
                      data-testid="weekly-task-save"
