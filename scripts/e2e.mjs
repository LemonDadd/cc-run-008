...
// ============ 玩家 A 全流程 ============
{
  ...
  await test('心情课：12 色各有情绪词，点进一色同屏出现大色块+情绪词+句子', async () => {
    await page.goto(BASE + '/#/mood');
    await page.waitForSelector('[data-mood="red"]');
    assert((await page.locator('[data-mood]').count()) === 12, '应有 12 张心情卡');
    await page.click('[data-mood="red"]');
    await page.waitForSelector('text=热闹');
    await page.waitForSelector('text=/大红灯笼/');
    const block = await page.locator('.rounded-\\[44px\\]').first().boundingBox();
    assert(block.height >= 0.6 * 950, `心情课大色块高 ${Math.round(block.height)}px，不足视口 60%`);
    // 朗读按钮存在且点击不报错（无声环境静默降级，页面照常能用）
    await page.click('[data-testid="mood-speak"]');
    await sleep(200);
    // 蓝色 = 安静
    await page.goto(BASE + '/#/mood/blue');
    await page.waitForSelector('text=安静');
    await page.waitForSelector('text=/平静的大海/');
  });
  ...
  await test('色温页：滑条靠左罩暖色写黄昏，靠右罩冷色写清晨', async () => {
    ...
    const setSlider = (v) =>
      page.locator('[data-testid="temp-slider"]').evaluate((el, val) => {
        const setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
        setter.call(el, val);
        el.dispatchEvent(new Event('input', { bubbles: true }));
      }, String(v));
    const overlay = () =>
      page.locator('[data-testid="temp-overlay"]').evaluate((el) => {
        const cs = getComputedStyle(el);
        return { bg: cs.backgroundColor, opacity: parseFloat(cs.opacity) };
      });
    // 靠左：暖色罩 + 黄昏
    await setSlider(0);
    await page.waitForSelector('text=更像黄昏');
    await sleep(250);
    const warm = await overlay();
    const wm = warm.bg.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
    assert(Number(wm[1]) > Number(wm[3]), `靠左应为暖色罩（红>蓝），实际 ${warm.bg}`);
    assert(warm.opacity > 0.3, `靠左暖色罩应明显，实际不透明度 ${warm.opacity}`);
    // 靠右：冷色罩 + 清晨
    await setSlider(100);
    await page.waitForSelector('text=更像清晨');
    await sleep(250);
    const cool = await overlay();
    const cm = cool.bg.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
    assert(Number(cm[3]) > Number(cm[1]), `靠右应为冷色罩（蓝>红），实际 ${cool.bg}`);
    assert(cool.opacity > 0.3, `靠右冷色罩应明显，实际不透明度 ${cool.opacity}`);
    // 中间：不冷不热，几乎无罩
    await setSlider(50);
    await page.waitForSelector('text=不冷不热');
    await sleep(250);
    assert((await overlay()).opacity < 0.05, '居中应基本无色罩');
    // 换场景照常可用
    await page.click('[data-scene="sea"]');
    await sleep(150);
  });

  await test('画廊：选中作品可打印一页（昵称+色块+那句说明）', async () => {
    await page.goto(BASE + '/#/gallery');
    await page.waitForSelector('text=我的作品');
    ...
    await sleep(400);
    assert((await page.evaluate(() => window.__printed)) === 1, '应调起浏览器打印 window.print()');
    // 打印介质下：打印页可见，且包含昵称、色块、那句说明
    await page.emulateMedia({ media: 'print' });
    const sheet = page.locator('[data-testid="print-sheet"]');
    assert(await sheet.isVisible(), '打印时打印页应可见');
    const txt = await sheet.innerText();
    assert(txt.includes('小明'), '打印页要有孩子昵称');
    assert((await sheet.locator('[data-testid="print-swatch"]').count()) >= 1, '打印页要有用到的色块');
    assert(/“.+”/.test(txt), '打印页要有作品里那句说明');
    await page.emulateMedia({ media: 'screen' });
    await page.getByRole('button', { name: '关闭' }).click();
  });

  await test('家长布置本周观察任务，孩子首页完成得 1 星并打勾', async () => {
    await page.goto(BASE + '/#/parent');
    await page.waitForSelector('[data-digit="1"]');
    await enterPin(page, '0000');
    await page.waitForSelector('text=学习总览');
    await page.getByPlaceholder(/找出三种暖色/).fill('找出三种暖色');
    await page.click('[data-testid="task-save"]');
    await page.waitForSelector('text=进行中');
    // 回首页：任务出现，完成后打勾且星星 +1
    await page.goto(BASE + '/#/home');
    await page.waitForSelector('text=找出三种暖色');
    const before = Number((await page.locator('.text-amber-500').first().innerText()).replace(/\D/g, ''));
    await page.click('[data-testid="task-done"]');
    await page.waitForSelector('text=✓ 已完成');
    await sleep(300);
    const after = Number((await page.locator('.text-amber-500').first().innerText()).replace(/\D/g, ''));
    assert(after === before + 1, `完成任务应 +1 星：${before} -> ${after}`);
    // 刷新后完成状态仍在（IndexedDB 持久化）
    await page.reload();
    await page.waitForLoadState('networkidle');
    await page.goto(BASE + '/#/home');
    await page.waitForSelector('text=✓ 已完成');
...
// ============ 两名儿童切换进度不串 ============
{
  ...
  await test('本周任务与星星在两个玩家间不串', async () => {
    // 当前是 B（阿二）。进家长面板，给 A（阿大）布置任务
    await page.goto(BASE + '/#/parent');
    await page.waitForSelector('[data-digit="1"]');
    await enterPin(page, '0000');
    await page.waitForSelector('text=学习总览');
    await page.getByRole('button', { name: /阿大/ }).click();
    await page.getByPlaceholder(/找出三种暖色/).fill('找出三种暖色');
    await page.click('[data-testid="task-save"]');
    await page.waitForSelector('text=进行中');
    // 以阿大身份进入首页：看到任务并完成
    await page.getByRole('button', { name: /以这个孩子身份进入/ }).click();
    await page.waitForURL(/#\/home/);
    await page.waitForSelector('text=找出三种暖色');
    await page.click('[data-testid="task-done"]');
    await page.waitForSelector('text=✓ 已完成');
    // 切到 B（阿二）：看不到 A 的任务和完成状态，星星不被串
    await page.goto(BASE + '/');
    await page.getByRole('button', { name: /阿二/ }).first().click();
    await page.waitForURL(/#\/home/);
    await sleep(500);
    assert(!(await page.isVisible('text=找出三种暖色')), 'B 不应看到 A 的任务');
    assert(!(await page.isVisible('text=✓ 已完成')), 'B 不应看到 A 的完成状态');
    const bStars = (await page.locator('.text-amber-500').first().innerText()).trim();
    assert(/⭐ 0/.test(bStars), 'B 的星星不应被 A 串号，实际 ' + bStars);
    // 切回 A：任务完成状态还在
    await page.goto(BASE + '/');
    await page.getByRole('button', { name: /阿大/ }).first().click();
    await page.waitForURL(/#\/home/);
